...
### 2026-09-05 - Frontend overhaul brief and visual exploration

- Read the linked Codex task `Review codebase structure`, including its three-PR proposal and staging acknowledgement-loss report, and reconciled key findings against the current checkout. Draft snapshot/retry defects and combined Failed/Unknown counters remain; some completed-screen wording has already improved. The campaign-list simplification is present in local commit `1122ec6` and must not be duplicated.
- Added `docs/FRONTEND_REFINEMENT_PLAN.md` with a proposed stable core architecture, limited correctness/diagnostics/API exceptions, frontend workflow hypotheses, integration sequence, and acceptance evidence. This is a recommendation, not an approved hard backend freeze or a completed usability audit. Updated the agent guide with the user's frontend intent.
- Used Product Design context and ideation plus built-in Image Gen to produce three separate desktop concepts in response to the user's request to compare the current identity with a new identity. Directly inspected and attached the approved mapping and dashboard mocks. The displayed order is 1 Familiar Paper, 2 Open Desk, 3 Clear Signal. Each uses the same imported-recipient scenario and a clear path to message composition; no direction has been selected for implementation.
- The generated concepts were inspected and displayed inline as brainstorming previews. They do not establish usability, accessibility, or functioning application behavior. No application code changed in this planning/ideation turn, no deployment or mail operation ran, and `git diff --check` passed. The previous 199-test result belongs to the preceding application change, not a new test run.

### 2026-09-05 - Familiar Paper selection and data-first template choice

- The user preferred the first displayed concept, Familiar Paper, while clarifying that a New send should import its spreadsheet before choosing a saved template or composing from scratch. Saved its selected recipient screen under `mock-images/refinement/01-recipients-familiar-paper.png` and updated the agent guide and refinement brief.
- Generated and inspected one matching Message-screen continuation using the exact selected image as the reference. It shows a saved-template selector and write-from-scratch action after import, readable dynamic values, optional attachments, collapsed sending settings, and a path to final review. Remaining recipient issues stay visible while composing. The continuation was displayed for feedback and is not implemented.
- Recommended preserving independent template management and an optional preselected-template reuse shortcut. The default template chooser must preserve imported recipients and validate the selected template rather than prefetching every template version.
- Documentation and selected-reference asset only; `git diff --check` passed. No application test rerun, provider interaction, deployment, or mail submission occurred.
...
### 2026-09-05 - Template save, picker, and field-resolution concepts

- The user accepted the Familiar Paper Message screen and asked how saving, selecting earlier templates, and mismatched dynamic fields should work. Saved the accepted Message reference at `mock-images/refinement/02-message-familiar-paper.png` and updated the guide and refinement plan.
- Generated and inspected three connected states using that exact Message reference: Save your message (new template versus explicit update), Choose a saved template (search/list, selected preview, and compatibility check), and Connect your message values (suggested Name-to-Full-name mapping plus unresolved Venue). These are proposed workflow states, not alternative identities or implemented screens.
- The proposal preserves the current imported spreadsheet when applying a template, retains unresolved dynamic tokens visibly, distinguishes missing columns from empty cells, and blocks sending until required values are resolved. Template changes for one send remain separate from an explicit saved-template update; spreadsheet rows and temporary attachments are not reusable template content.
- Source inspection confirmed that the existing saved-flow hydrator resets workbook/attachments and the template-version API helper advances the reusable flow pointer during campaign preparation. Both need deliberate lifecycle changes before the proposed UX can be promised. Added these implementation implications to the refinement plan without changing the runtime contract.
- `git diff --check` passed. Changes are design documentation and the accepted reference image only. No application test rerun, remote data access, deployment, provider interaction, or mail submission was performed in this design turn. Preserved the concurrently appended campaign-list production-release entry.

### 2026-09-06 - Low-value code audit and dead-code cleanup

- Inspected every major project directory: root package/build/test configuration; `.github`; `scripts`; `worker`; all nine `migrations`; `src/app` routing, components, hooks, state, helpers and styles; `src/client`; `src/domain`; each `src/server` API, attachment, authentication, database, Microsoft and Queue module group; `tests` fixtures/stubs; `public`; `mock-images`; and `docs` including the archive and existing refinement work. Inventoried local/generated `.codex-tmp`, `.playwright-cli`, `.wrangler`, `dist`, `node_modules`, `output`, and `outputs` without purging them. Dependency internals and local secret files were excluded from the code audit.
- Removed eight unimported re-export entrypoints: `src/client/data.ts`, `src/domain/{campaign,contracts,recipient}.ts`, `src/server/auth/index.ts`, `src/server/database/{index,repositories}.ts`, and `src/server/queue/adapter.ts`. Repository-wide import/reference checks found no consumers; the package is private. Removed unused convenience aliases and their obsolete caller/compatibility comments in browser, auth, attachment, and Graph helpers; retained the active implementations and adapters.
- Removed the unused in-memory lifecycle from `src/domain/state.ts`. Its only callers were its own tests and the deleted barrels; actual state changes run through D1 conditional SQL and the mailbox/Queue coordination code. Preserved `makeSendKey` exactly, replaced three tests of the unused model with two tests for stable/distinct send keys and invalid identity/row rejection, and kept D1 claim, lease, recovery, and unknown-outcome tests.
- Removed unused mapping/preview helpers, the Graph constructor wrapper, and the one-call spreadsheet import wrapper. The import test now calls the same parser and table selector as the UI, preserving CSV format detection and invalid XLSX coverage. Removed the obsolete string-selection helper and its two exclusive tests; the active editor uses DOM Range and source-textarea selection. Removed one redundant negative mapping assertion already covered by exact object equality.
- Removed the unused browser CSV serializer, download wrappers, and dedicated row type. The UI already downloads the authenticated Worker export. Moved the existing quoting/formula-injection regression to `src/server/api/security.test.ts`, where it now exercises the real `jobCsv` serializer. No runtime behavior, migration, configuration, visual layout, dependency, or provider adapter contract changed. Source/test diff: 26 files, 44 insertions, 643 deletions, 599 net lines removed.
- Retained thin runtime repository/Queue/mail adapters, resource-specific OAuth and key-rotation support, browser versus Worker validation, migration guards, staging build isolation, test socket injection, CSV QA fixture, and both referenced public assets. Their runtime, operational, or accepted-behavior roles justify them. Existing frontend design/progress changes were preserved.
- Baseline `npm test` passed TypeScript, both builds, and 199 tests in 24 files. Focused command `npx vitest run src/client/client.test.ts src/domain/state.test.ts src/server/api/security.test.ts src/server/auth/auth.test.ts src/server/microsoft/microsoft.test.ts src/server/attachments/policy.test.ts src/server/database/d1-mailbox-delivery.test.ts src/server/queue/campaign-tick.test.ts src/app/App.test.tsx` passed 107 tests in nine files. Post-cleanup `npm test` passed TypeScript (including configured unused-local/parameter checks), both builds, and all 196 tests in 24 files. The three-test net reduction is two obsolete editor tests plus three unused-model tests replaced by two live send-key tests; CSV coverage moved without reducing its test count.
- `npm run lint` was attempted and exited 1 because this repository has no lint script or linter configuration. No replacement lint framework or redundant package script was added. `git -c core.safecrlf=false diff --check` passed. A final relative-import scan resolved all 556 references across 152 remaining code files with no missing modules. Existing Zod annotation and large-client-chunk build warnings remain.
- SHA-256 comparison of all three emitted JavaScript/CSS artifacts matched the baseline byte for byte: client JavaScript `FC10E271AEBD58EFF13073C3A377C4AAA81650D54DAFFD8FF895E0299E696596`; client CSS `377A29162B4E311C605BD96542508954D6CB34DF39CD35AD0F099CA2303F50C6`; Worker JavaScript `22B6D66A21036C27589F6795CA9F805177EB0313E4BF8087A9D96F69E72BF0F3`.
- Started the local full-stack server on `http://127.0.0.1:5174/` and verified the signed-out landing and Microsoft entry controls in the in-app browser. Local HTTP checks passed: landing 200 HTML, client entry 200 JavaScript, logo 200 PNG, unauthenticated `/api/me` 401 JSON, and unknown API GET/POST 404 JSON. No remote deployment, migration, Microsoft sign-in, campaign mutation, attachment operation, or mail submission occurred.

### 2026-09-06 - First-principles cleanup with existing workflows preserved

- Rechecked the remaining abstractions against the existing import, template, review, and send use cases. Deleted the unused domain template renderer and campaign validator, including their exclusive types and tests. The browser renderer and campaign validator remain the active paths; existing client tests cover escaping, sanitization, required mappings, and empty values, and the live renderer test now also asserts missing-field reporting.
- Reused the identical email normalization, email syntax, placeholder extraction, and default recipient limit from the domain validation module. Retained separate client and server row validation because their diagnostics and invalid-input handling differ. No validation result or default separator was changed.
- Centralized the common attachment extension/MIME table and five-file/20-MiB limits in `src/domain/attachments.ts`. The browser picker derives its accept list from that table; upload policy and SMTP limits use the same constants. Removed a no-op MIME alias and redundant local variable while preserving the browser's permissive type hints and the Worker's stricter extension/type and content checks.
- Replaced two copies of the SQLite-backed D1 test adapter with `tests/helpers/sqlite-d1.ts`, preserving real transactions, rollback, migration ordering, and all campaign-create and mailbox-delivery assertions. Computed spreadsheet mappings and their issues once per draft change, and removed an always-true branch around template-version creation without changing the save/send lifecycle. This pass removes 279 net source/test lines across 18 files, relative to the preceding cleanup.
- Retained the D1/Queue/mail adapter boundaries, conditional claims, unknown-outcome handling, separate resource authorization, and migration/deployment safeguards because they enforce accepted behavior. The previously documented draft snapshot and creation-retry defects remain; correcting those lifecycle contracts is separate work. Existing frontend design documents and selected mocks were preserved.
- Verification: the focused tests passed 94 tests across nine files. `npm test` passed TypeScript, both production builds, and all 193 tests across 23 files. The reduction from 196 tests is three tests dedicated to deleted, unreachable domain implementations; active workflow and backend assertions remain. Existing Zod annotation and client chunk-size warnings remain non-blocking.
- An ephemeral before/after comparison bundled captured pre-change sources and current sources and passed 1,390 comparisons covering email parsing, placeholders, row/campaign validation, attachment extensions and MIME declarations, and size/count limits. The comparison script was removed afterward. `git -c core.safecrlf=false diff --check` passed. The emitted CSS SHA-256 remains `377A29162B4E311C605BD96542508954D6CB34DF39CD35AD0F099CA2303F50C6`.
- Refreshed and visually inspected the local signed-out landing in the in-app browser. Local HTTP checks passed: landing 200 HTML, unauthenticated `/api/me` 401 JSON, and unknown API GET/POST 404 JSON. No Microsoft sign-in, authenticated end-to-end mail test, remote deployment, migration, campaign mutation, attachment operation, or mail submission ran.

### 2026-09-06 - Architecture assessment and refactor recommendation

- Reviewed the accepted use cases, architecture/decision and operations/testing documents, existing refinement plan, and current implementation at `1122ec6` plus local cleanup changes. Added `docs/ARCHITECTURE_REVIEW_2026-09-06.md` with evidence, priorities, a proposed incremental sequence and explicit limits. Recommendation: retain the modular Worker/D1/Queue architecture and existing delivery safeguards; correct lifecycle and recovery contracts before broadening the rollout. No architecture change or rewrite was implemented.
- Reproduced the existing draft-to-campaign mismatch and same-key/different-template-version retry with the real DraftProvider and preparation hook against mocked API methods. Also reproduced two SMTP authorization compositions using the actual adapter and queue processor with the existing synthetic queue harness: OAuth invalid-grant produces a terminal failed recipient while the campaign remains running; missing-token AuthFlowError becomes unknown before the SMTP send method is called. These remain unresolved and are recorded as high-priority lifecycle/recovery work.
- Identified deliberate template publication and atomic save semantics, consistent Failed/Unknown presentation, safe correlation/stage diagnostics, narrower application/state boundaries, integrated failure-path coverage, lazy loading, history pagination and measured capacity/retention/restore policies as follow-up investments. Existing conditional claims, mailbox budget/attempt ledger, wake tokens, immutable attachments, resource-specific OAuth and server-side validation remain justified by the use cases.
- Verification: `npm test` passed TypeScript, both production builds, and all 193 tests in 23 files. `npx vitest run src/app/hooks/architecture-review.probe.test.tsx src/server/queue/architecture-review.probe.test.ts` passed four reproduction probes across two files; both temporary probe files were removed afterward. Existing Zod annotation and large-client-chunk warnings remain. Client JavaScript: 1,436.69 kB, 417.16 kB gzip. `git -c core.safecrlf=false diff --check` passed.
- Documentation only remains from this review; all pre-existing application and planning edits were preserved. No server deployment, remote migration, Microsoft authorization, OneDrive operation, actual mail submission, authenticated visual journey or load test ran. Passing tests do not establish production capacity or actual delivery.
