...
### 2026-09-05 - Current-code maintainability review

- The user reports that the latest staging version works in real use. Recorded as user-confirmed functional verification; no detailed account, inbox or attachment matrix was supplied and no independent mail test ran in this review.
- Reviewed frontend, client utilities, API, domain validation, persistence/mail/queue boundaries, styles and test consumers at HEAD `dc1a0eb` (runtime `812ab2b`). Findings and a proposed sequence are in `docs/CODE_QUALITY_REVIEW.md`: unused helpers and test-only implementations, duplicate CSV export with coverage on the unused path, obsolete draft fields and optional provider fallbacks, duplicate API/mapping/edit-loading work, and accumulated CSS overrides. Retain delivery/ownership/security safeguards and move useful assertions onto runtime paths.
- Confirmed the separate monitor issue where Unknown is included in Failed outside the campaign-level-failure branch. Keep that behavior correction and performance/draft-recovery work separate from deletion-only cleanup.
- `npm run test:unit` passed all 29 files and 226 tests. This checkpoint changes review documentation only. No application cleanup, remote data mutation, mail send or deployment was performed.

### 2026-09-05 - Focused code-quality cleanup implementation

- Implemented the user-approved cleanup on `chore/code-quality-cleanup`, based on frontend checkpoint `16be4a6`. Separate commits remove unused paths and repair export coverage, simplify active draft/API plumbing, and consolidate presentation styles. The cleanup PR targets `feat/frontend-refinement` so its diff excludes the earlier frontend implementation.
- Removed unused client helpers, the duplicate client CSV exporter and obsolete template renderer, write-only draft fields and unused count projections. Live CSV route tests cover ownership, 500-row pagination, quoting and formula protection. Shared validation primitives preserve invalid-address handling and rendering/security boundaries.
- Unified JSON and multipart request handling with CSRF, credentials, browser-generated boundaries, error redaction and 204 coverage. Required provider-owned snapshot/test/preparation state, retained coalescing and locking, computed spreadsheet mappings once, removed the duplicate template-edit fetch, and rejected incomplete attachment metadata rather than inventing ready state.
- Replaced stacked base/responsive/polish/refinement stylesheets with component/route owners, retained shared tokens, removed unused selectors and the hidden step-count element, and formatted touched JSX/CSS. Fixed a mobile step-label regression detected by before/after captures before final validation. Existing 641-680px compact-label behavior is retained; redesign and behavior changes remain outside this cleanup.
- `npm run check:staging` passed TypeScript, production and staging builds, 222 tests in 29 files, both Wrangler deployment dry runs and isolated staging target validation. `git diff --check` passed. Existing dependency annotation and large client chunk warnings remain; route/parser lazy loading and the Unknown/Failed behavior fix remain separate work.
- Chrome synthetic E2E verified a 48-row XLSX import (46 valid, 2 flagged), saved-template preview/reuse, save cancellation and duplicate-name failure, direct saved-template editing without a recipient file, two attachment types (27 and 45 bytes), Review metadata, sample switching, skipped rows and confirmation gating. Before/after captures cover Recipients, Message and Review at 1920, 1440, 1024 and 390px plus empty, picker and save states. Reduced motion and keyboard dismissal passed. Current verification produced no browser warnings or errors; earlier hot-reload diagnostics during incomplete edits were excluded after reload. Local fixtures block provider-bound actions, and no real mail was sent.

### 2026-09-05 - Cleanup PR and staging deployment

- Opened PR #8, `https://github.com/kyzer1023/MailFlow/pull/8`, against `feat/frontend-refinement`. Runtime candidate `f4e847447643164b4fcf12f78591751c5ff9cf0b` passed GitHub Verify run `33963232734` as well as the local staging gate.
- Staging D1 reported no pending migrations. Deployed through `dist/client/mailflow/wrangler.staging-validated.json` with the exact candidate SHA in the release message. Worker version `95ea7b1a-bc26-4793-95c4-cd53e29da704` receives 100 percent of staging traffic, confirmed at 19:25 MYT. Producer/consumer, D1, DLQ, namespace, SMTP settings and the hourly schedule remain isolated to staging.
- Hosted landing returned 200. JavaScript `index-C2tsIMFS.js` and stylesheet `index-D64aAOdM.css` returned 200 and matched local candidate byte hashes. Unauthenticated `/api/me` returned 401 JSON; unknown API GET/POST returned 404 JSON. The Microsoft authorization redirect returned 302 with the staging callback and delegated SMTP scope. No local fixture script or identity appears in the deployed assets.
- Chrome displayed the hosted signed-out landing and remains open for the user. Temporary viewport and reduced-motion overrides were cleared. No production deployment, campaign action, OneDrive operation or real-mail submission ran. The separate Cloudflare Workers Builds check was still pending when this deployment checkpoint was recorded; it is not the successful GitHub Verify run or the verified direct staging deployment.

### 2026-09-05 - Template connection feedback and focus correction

- The user reports completing the previous manual verification checklist successfully and confirms that differently named columns already resolved correctly in Review. Their remaining feedback concerns invisible completion feedback and the unexpected subject focus after resolving the final field.
- Kept the Message value connections panel and its column selectors mounted after resolution. Each row shows its actual selected column and a green Connected state, with an accessible overall completion message. Members can inspect, change or clear a connection; clearing it restores prerequisite gating. Template-value names and stored rendering semantics remain unchanged.
- Removed automatic focus advancement to the subject. Native column selection retains focus. Choosing a suggested column focuses that same row's surviving selector without scrolling; replacing a value with text returns focus to the stable sidebar instead of the subject.
- `npx vitest run src/app/App.test.tsx` passed all 25 UI tests. Extended multi-field and fixed-text coverage and added a regression for completing the last field, focus retention, clearing/reconnecting, Review values after remapping, and persistence when returning from Review. `npm run check:staging` passed TypeScript, production/staging builds, all 223 tests in 29 files, both packaging dry runs and staging target validation. `git diff --check` passed. Existing dependency annotation and client chunk warnings remain.
- Chrome was unavailable in this session, so browser verification used the in-app browser with synthetic XLSX data. Keyboard selection and the suggestion action kept focus on the mapping selector; clearing blocked Review; the email preview contained the correct resolved name. Desktop 1440x900, tablet 1024x768 and mobile 390x844 captures showed readable retained connections and no horizontal document overflow. The only observed console error was traced through CDP to the browser's isolated `browser-page-preload.js` script, not the app execution context. No provider-bound action or real-mail submission ran.

### 2026-09-05 - Mapping feedback staging deployment

- Added the focused feedback correction to PR #8 and deployed exact runtime candidate `85598ead978a76e381ac7d2f1f14a21e7949818f` using the validated staging configuration. Worker version `104f0707-5b3d-4e06-8d7a-aec0f3431f51` receives 100 percent of staging traffic, confirmed at 22:01 MYT. Staging D1 has no pending migrations; the isolated Queue, D1, namespace and hourly schedule are retained.
- Hosted JavaScript `index-BNQgrTP8.js` and stylesheet `index-Dg7EimHz.css` matched the checked local build byte for byte. Landing 200, unauthenticated API 401 JSON, unknown API GET/POST 404 JSON and the staging Microsoft SMTP authorization redirect checks all passed. Synthetic fixtures remain absent from hosted assets. The in-app browser opened the hosted landing; temporary viewport overrides were cleared.
- No production deployment, campaign mutation, attachment upload to Microsoft or real-mail submission ran. The final documentation commit records this deployment without changing its runtime.

### 2026-09-05 - Approved main integration

- The user approved PR #8 for main after the manual staging verification and mapping-feedback correction. Retargeted the PR from `feat/frontend-refinement` to `main`, bringing the full approved recipient-first interface, explicit template saving/reuse, persistent mapping feedback and maintainability cleanup into one integration.
- Confirmed current `origin/main` (`c8aef8d`) is an ancestor of the reviewed branch, with no source conflict or runtime change needed. Runtime remains the staging-verified `85598ea`; subsequent commits only record verification and release evidence. The existing application checkout and other worktrees are preserved.
- The deployed runtime passed GitHub Verify and Cloudflare Workers Builds. All 223 local tests, TypeScript, both builds, both deployment dry runs and staging smoke checks passed as recorded above. The documentation-only `709350f` commit passed GitHub Verify but its separate Cloudflare build failed without error text; no failed build is represented as passing.


### 2026-09-05 - Results reporting, manual delivery evidence and sanitized diagnostics

- Based the isolated `feat/results-verification-diagnostics` worktree on freshly fetched `origin/main` at `146c5120202dddf544fb53b54d54d68dc072ce3e`; re-fetched before final verification and confirmed main had not moved. No other worktree changes were touched.
- Running and completed monitors now show Unknown and recipient failure separately, with processed-row feedback and receipt-before-resend guidance. Completion explains processing completion rather than inbox delivery. History retains separate provider outcomes, includes skipped rows and shows manual verification separately; dashboard attention grouping and raw CSV statuses remain intact.
- Documented the contract in ARCHITECTURE before implementation. Forward-only migration `0010_manual_delivery_verification.sql` adds immutable owner/timestamp/optional 500-character note evidence. An owner-scoped conditional update and atomic audit trigger preserve original Unknown outcome, provider metadata, processing timestamps, attempts and mailbox-budget ledger. The authenticated same-origin/CSRF mutation validates confirmation and note bounds, rejects forged fields and returns first-write evidence on repeated/concurrent actions. Notes stay outside audit and diagnostic logs and receive CSV formula protection.
- Recipient and campaign times show seconds with MYT (UTC+8). Elapsed duration includes waits. Observed throughput uses at least three terminal non-skipped outcomes, remains distinct from configured maximum pace, and drives estimates only for active, non-waiting, recent processing. Paused, waiting, stale and terminal states make no remaining-time promise.
- SMTP diagnostics record fixed stages, timeout/socket-closure/socket-failure classification, elapsed milliseconds and a generated correlation ID linked to recipient or controlled test-send failure audits. DATA-terminator ambiguity, safe retry decisions and the 30-second per-operation timeout are unchanged. Uncaught API diagnostics retain generic browser errors and emit only fixed route/stage/classification, duration and an application-generated request ID returned in a response header. Arbitrary exception names/codes, stacks, payloads, URLs, addresses, credentials and message/OneDrive content are excluded.
- Verification commands: `npm ci` completed; focused Vitest runs passed; `npm run check:staging` passed TypeScript, production/staging builds, all 253 tests in 33 files, both Wrangler dry runs and staging-target validation. `npx wrangler d1 migrations apply mailflow-db --local --config wrangler.jsonc` applied all ten migrations successfully to this worktree's local D1. `git -c core.safecrlf=false diff --check` passed. An earlier check with `core.autocrlf=false` incorrectly treated Windows line endings as whole-file whitespace changes; the normal normalization-aware check is clean.
- `npm run dev -- --host 127.0.0.1 --port 4173 --strictPort` serves the local preview. In-app browser checks used synthetic intercepted data at 1440x900, 1024x768, 390x844, source-size 1672x941 and wide 1920x1080. Verified confirmation gating, note, error/retry, cancellation, focus, saved evidence, history, stopped/paused states and reduced motion. Fixed a seven-counter mobile grid conflict found in QA. Final console checks were empty; viewport/media overrides were cleared. See the dated section in design-qa.md and ignored local screenshot evidence.
- Existing dependency annotation and large-client-chunk warnings remain. Remote migration/deployment, live-campaign mutation and real-mail submission were not performed. Historical unknown outcomes cannot retroactively establish timeout versus closure; manual receipt reports remain distinct from SMTP evidence. Migration 0010 must precede any future deployment of this code.

### 2026-09-05 - Results verification staging release

- Following explicit user authorization, dispatched the serialized staging workflow for PR #9 runtime candidate `568e05db37496e43e1990baff5d57eca75f05dbd`. GitHub run `33976146288` passed all 253 tests in 33 files, TypeScript, production/staging builds and both deployment dry runs, then applied `0010_manual_delivery_verification.sql` and deployed the validated staging configuration.
- Worker version `434d7b49-0190-4bb8-9378-63068343ca21` receives 100 percent of staging traffic, deployed at 23:54 MYT. The deployment message identifies the exact candidate SHA. Remote staging migration listing reports no pending migrations; the hourly schedule remains `15 * * * *`.
- Non-sending hosted checks passed: landing and hashed JavaScript/CSS returned 200, JavaScript includes the new verification action, unauthenticated `/api/me` returned 401 JSON, unknown API GET/POST returned 404 JSON, and Microsoft authorization returned 302 with the staging callback and delegated SMTP scope. The redirect was not followed. No production deployment, campaign mutation, or real-mail submission ran.
- User will perform manual verification: compare Unknown versus failed counts, confirm an independently received Unknown row and refresh/export its immutable evidence, inspect MYT timestamps and observed processing speed on a small campaign, and check paused/completed estimates. Historical Unknown rows cannot supply the new SMTP diagnostics retroactively. This documentation checkpoint does not change the deployed runtime.

### 2026-09-06 - Explicit finished results and verification layout

- Traced the reported Completed badge to direct rendering of the scheduler state. `completeIfExhausted` intentionally completes processing when no pending, claimed or sending jobs remain, including when terminal Unknown or failed outcomes exist. Preserved this state machine and all historical records; result labels now derive from existing counts on both detail and history screens.
- Replaced generic Completed with receipt-unverified, recipient-failure, manually verified, skipped, or processing-finished wording. Detail progress lists every outcome count. Moved verification actions beneath a separate wrapping note, and removed the active Sending highlight from finished campaigns.
- `npm run check:staging` passed all 256 tests in 34 files, TypeScript, both builds and deployment dry runs. Initial sandboxed Vite execution could not resolve its config; the approved external execution passed. Browser synthetic checks at 1682x868 and 390x844 confirmed result text, confirmation gating, post-verification failure visibility, separate action spacing and no document overflow; console warnings/errors were empty. No recipient data was changed or mail sent.
- Staging workflow run `33982441524` deployed runtime `9798f1d7fd0af78071c3efcc5026266f52f8cc87` at 01:57 MYT on 2026-09-06, Worker version `0f6c0e3d-865f-4756-a3f1-e0d207c92423`. No migrations were pending. Hosted candidate wording/assets, unauthenticated API, unknown GET/POST API routes and staging SMTP redirect checks passed. Additional 1440x900 and 1024x768 browser checks passed, and history shows the same derived result label. No production deployment or live campaign mutation ran.

### 2026-09-06 - Viewer-local timestamps
- Website timestamps now use the browser locale and timezone with seconds and an explicit GMT offset. Campaign details, recipient updates, verification evidence and history share the formatter. Scheduler notices localize both ISO timestamps and existing Malaysia-formatted server text; API errors use the same conversion so test-send waits are consistent. Stored UTC timestamps, exports, backend scheduling and elapsed calculations remain unchanged.
- Browser emulation verified New York GMT-4 and India GMT+5:30, including date rollover and unchanged elapsed duration; overrides were reset. Tests cover browser defaults, fractional offsets, winter/summer daylight saving, legacy/ISO notices, duplicate-time suppression and invalid values. All 260 tests passed; release build/dry-run results follow in the deployment checkpoint.
- Full `npm run check:staging` passed both builds/dry runs and 260 tests. Staging run `33982790090` deployed runtime `8ed5ff49ee37d65bf6806bf5eced2c386ce6afc0` at 02:03 MYT, Worker version `4c98c6ee-593f-46d5-a272-e9ee858b0c36`. No migrations were pending. Hosted asset, unauthenticated API, missing API GET/POST and staging SMTP redirect checks passed. No production deployment or mail action ran.

### 2026-09-06 - Readable status pills
- Increased shared pills to a 32px minimum height, 13px text, 7px/12px padding and stable nonshrinking dots through centralized tokens. Completed campaigns with no failed, unknown or skipped rows now say Accepted by Microsoft, preserving the acceptance-versus-receipt distinction. Other result labels and backend states remain unchanged.
- Desktop screenshot comparison against the supplied status-column image verified larger consistent badges. Mobile QA revealed pre-existing fixed-column squeezing; the campaign table now has a 640px minimum width within its horizontal scroll container, with a focusable named region for keyboard scrolling. Long badges wrap at readable word boundaries instead of overlapping adjacent results. Synthetic captures are in artifacts/local/results/status-pills-desktop.png and status-pills-mobile.png. Browser logs empty; viewport override reset.
- Full local release gate and staging workflow `33983301341` passed all 260 tests, TypeScript, builds and dry runs. Runtime `b12ee57c3837b8dbb411706dfb8e91fc43f78b7b` deployed at 02:13 MYT with Worker version `f89b8f8e-ae17-49f9-981c-6ec647d694a1`. No pending migrations; hosted assets and non-sending API/OAuth checks passed. Production and live campaigns were not mutated.

### 2026-09-06 - Manual verification feedback and scheduling investigation
- User reports that manual delivery verification passed. Their screenshots show a newer five-recipient campaign progressing while an existing 108-recipient campaign waited after 77 accepted rows, then the existing campaign resumed. This is user-observed evidence, not an independently inspected live-mail audit.
- Read-only code investigation found no explicit newest-first campaign priority: independent campaign ticks compete for the mailbox lease. MAILBOX_LEASE_MS is a fixed five minutes; a lease collision persists the lease expiry as the waiting campaign's next-attempt time. Completing another campaign's attempt releases the mailbox but does not directly wake other waiting campaigns early. History maps running state without scheduler waiting context, and raw recipient notes bypass browser-local scheduler formatting. These mechanisms are consistent with the reported symptoms; exact live attempt timing was not queried.
- Proposed follow-up for user review: explicit per-mailbox campaign ordering and prompt handoff; consistent Queued/Sending/Waiting display; clear Completed campaign summary while retaining recipient acceptance evidence; terminal audited cancellation that prevents new submissions and lets in-flight outcomes settle. No scheduler, cancellation, deployment or live campaign changes were made during this investigation.


### 2026-09-06 - FIFO campaign turns, cancellation, and consistent results

- Implemented the user-approved mailbox FIFO on the results/verification PR. D1 assigns durable monotonic turns on start and resume; resume joins the back. Only the head reserves/consumes wakes, claims recipients, and crosses the provider boundary. Followers stay Queued without timer polling. Completion, pause, cancellation, and terminal attachment failure hand off after current submission settles, respecting mailbox pace, backoff, pending-row retry deadlines and budget. A lease collision now waits for release instead of scheduling a five-minute wake; scheduled recovery remains fallback for missed events/publication.
- Added forward-only migration `0011_campaign_turns_cancellation.sql`. Backfill prioritizes provider-bound work, then reservations and queue time, preserves the head's published wake, clears follower wakes, and releases only proven pre-submission follower claims/reservations. Database guards cover the brief old/new Worker overlap. Accepted/Unknown ledger and row evidence are preserved.
- Added owner-scoped authenticated/same-origin/CSRF-protected acknowledged cancellation with immutable first-request/completion timestamps and atomic audit triggers. Pre-boundary cancellation prevents submission; an existing provider-bound campaign or test attempt keeps its actual result. Cancelling becomes Cancelled after settlement. Remaining rows stay original pending evidence, display Not sent, and cannot resume; new tests against cancelled campaigns are rejected. CSV adds separate campaign state, cancellation timestamps and unsent reason.
- History and detail share Queued/Sending/Waiting/Paused/Cancelling/Cancelled presentation, with exceptional waiting reasons and local timestamps. Normal pacing remains Sending. Fully accepted campaigns show Completed and a successful Microsoft-submission summary, retaining Accepted by Microsoft on recipient rows. Fixed the missed browser-timezone conversion in persisted recipient notes. Cancellation has explicit checkbox gating, dismissal, busy, error/retry and success/focus states.
- Regression coverage includes FIFO handoff/pacing, independent mailboxes, dormant followers, duplicate ticks during attachment loading, watchdog non-interference with fresh active work, pause/resume-to-back races, cancellation before/during submission, accepted/unknown/budget/manual-evidence preservation, first-write audit rollback/replay, cancellation authentication/ownership/schema checks, missing-handoff and provider-crash recovery, legacy migration backfill, CSV, completion wording and UI confirmation/focus.
- `npm run check:staging` passed TypeScript, production/staging builds, 295 tests in 36 files, both Wrangler deployment dry runs and isolated staging configuration validation. `npx wrangler d1 migrations apply mailflow-db --local --config wrangler.jsonc` passed; a separate fresh local persistence directory also applied all 11 migrations. Inspected local schema to confirm the turn view and seven FIFO/cancellation triggers exist. `git diff --check` passed. Existing dependency annotation and large client bundle warnings remain.
- Browser QA used the in-app browser because no Chrome surface was available. Reference-sized, 1440, 1024, 390 and 1920px views passed; cancellation error/retry and focus, mobile table keyboard scrolling, local-time notices under America/New_York, reduced motion and empty console checks passed. See the appended `design-qa.md` section and ignored `artifacts/local/fifo/` captures. No real mail, live campaign mutation, or production deployment ran during implementation/verification.
- Staging workflow `33986474733` passed verification and applied migration 0011, then deployed exact runtime `9969f0b6e0e35e5189606a1859dd0567cb44c970` at 03:15 MYT on 2026-09-06, Worker version `41cd1a8a-62db-4d18-bb84-dd70e64378f9`. Hosted JS/CSS SHA-256 matched the locally verified staging build. Landing, unauthenticated API, missing API GET/POST and delegated SMTP redirect checks passed; the hosted landing also rendered in the in-app browser. OAuth redirect was not followed and no real-mail submission or live campaign action ran. Production remains unchanged.

### 2026-09-06 - Correct trigger-aware campaign start and recovery

- User reported Confirm & start returning to idle on Review while a newly created campaign stayed Queued. Reproduced the underlying mismatch with the actual local Cloudflare D1 binding: one update plus a trigger insert reports `meta.changes = 2`, while SQLite `changes()` reports 1. The repository incorrectly required exactly one D1 change, so a committed FIFO start returned false before publishing its wake. The original SQLite test adapter hid the failure by reporting direct changes only. D1 metadata semantics are documented at https://developers.cloudflare.com/api/resources/d1/.
- Changed primary-key campaign mutation success checks to accept a positive change count. Unmatched owner/state conditions still return false. This also corrects pause, resume, completion and attachment-stop results with FIFO/audit triggers. Kept conditional SQL guards and all original outcome/budget evidence intact.
- An authenticated, acknowledged repeated start now returns the same queued/running/completed campaign. Queued replay requests the existing mailbox head's missing wake without adding a turn or replacing a reserved wake. Running/completed replay publishes nothing; stopped/cancelled campaigns cannot restart this way.
- Both SQLite integration adapters now emulate D1's total-change delta. Three existing FIFO tests failed under that corrected adapter before the fix, then passed. Added a fresh, isolated actual local D1 runtime test applying all migrations and checking owner rejection, start, first wake, pause, resume and irreversible cancellation. Added API replay and UI same-campaign retry/navigation coverage.
- `npm run check:staging` passed all 307 tests in 37 files, TypeScript, production/staging builds, both Wrangler dry runs and staging configuration validation. Actual local D1 and synthetic browser checks made no provider calls. Browser verification showed a retryable interrupted-start error and then navigation to the same campaign ID. No visible layout changed. Existing dependency annotation and bundle-size warnings remain. No new migration is required.
- Staging workflow `33987231797` repeated the 307-test gate and deployed exact runtime `9f83c2f6325562d2b55d1920f91319d323363212` at 03:30 MYT, Worker version `fc972c3e-6c94-431e-8e9d-8f45b9a61520`. No migrations were pending. Hosted asset SHA-256 comparison, landing render, unauthenticated API, unknown API GET/POST and staging SMTP redirect checks passed. No authenticated live campaign or mail action ran; the already-queued campaign can recover through acknowledged start replay, pause/resume, or the existing watchdog. Production remains unchanged.

### 2026-09-06 - Cancellation after the last submission

- Inspected the user-specified staging campaign with read-only D1 queries. Its fifth attempt became provider-bound at 03:35:03.867 MYT; cancellation was requested at 03:35:05.688; the final acceptance was recorded at 03:35:09.348; cancellation settled at 03:35:10.449. All five recipient and delivery-attempt outcomes are Accepted. The request and settlement audit events both exist. This establishes provider acceptance, not inbox delivery. Queries retrieved only states, counts, times and envelope counts; no content, addresses or tokens, and no live state was changed.
- Corrected presentation when a settled cancellation stopped no rows. Complete terminal counts now use normal completed-result presentation in detail and history: all accepted shows Completed; mixed results retain failure, Unknown and skipped warnings. Both views include "Cancellation was requested, but no rows were stopped." Missing counts, remaining pending rows or an unsettled attempt retain cancellation presentation. The detail page keeps the request timestamp and describes the audit evidence. Raw irreversible cancellation state, audit, recipient evidence, CSV and scheduler behavior remain unchanged; this also corrects existing campaign displays.
- Full `npm run check:staging` passed 311 tests in 37 files, TypeScript, both builds/dry runs and staging configuration validation. Regression tests cover the all-accepted edge case, mixed outcomes, incomplete counts, in-flight work, retained audit timestamp and unchanged raw state. Synthetic browser checks confirmed matching Completed/note in history and detail, with 1440, 1024 and 390px layouts using the existing scrollable table. No new migration or real send is required.
- Staging workflow `33987885517` passed and deployed exact runtime `c247ba44763a1eb3c6bea26bba2e176bc9c999cc` at 03:43 MYT, Worker version `9e2405ea-2d1f-4a38-8be8-a43755dd6dd2`. No migrations were pending. Hosted JS/CSS hashes matched the verified build, including the new cancellation note; landing/API/OAuth smoke checks passed. Production remains unchanged, and no live campaign data or sending action was modified.

### 2026-09-06 - Frontend loading performance and focused cleanup

- Completed the remaining review cleanup on an isolated branch based on merged results/FIFO work. Product layout and seven route modules now load on demand; ExcelJS loads only when a size/package-validated XLSX is parsed. Initial production JavaScript fell from 1,474.50 kB / 428.85 kB gzip to 264.61 kB / 83.88 kB gzip (about 80% less gzip). The optional ExcelJS chunk remains 940.37 kB / 271.39 kB gzip; its existing size warning is retained.
- Split attachment queue/file ownership and derived validation out of DraftProvider while preserving the public context contract, immutable preparation keys, serialized uploads and generation resets. Added route loading/error recovery without remounting the shared draft on navigation. Six regressions cover suspended/failed route recovery, upload serialization/retry, stale set/upload results, queue reset, and prepared-campaign removal protection.
- Centralized remaining literal colors, shared radii, stacking levels and common pixel spacing across the stylesheet modules. An independent expansion comparison against the base revision confirmed identical existing declaration values; CSS gzip increased from 14.36 kB to 15.64 kB. Desktop baseline and final recipients captures retain the same layout. Browser QA also found the existing mobile history Open column was hidden; campaign names now link to their results at every width, with visible keyboard focus.
- `npm test` passed TypeScript, production build, the new manifest/static-import and 110 kB gzip budget check, and all 317 tests in 39 files. `npx wrangler deploy --dry-run`, `npm run build:staging`, `npm run prepare:staging-config`, and the validated staging Wrangler dry run passed. Formatting did not change the built asset hashes. Existing dependency annotation warnings remain non-blocking.
- Chrome checked the actual built client with local synthetic API fixtures: landing requests only its entry bundle; CSV import requests no ExcelJS; XLSX import requests the deferred chunk and parses 48 rows correctly (46 ready, 2 flagged). Verified saved-template reuse, readable message values, CC chips/High importance, two ready attachment types with matching 27 B/45 B review totals, draft persistence across back navigation and file replacement, flagged-row skipping, acknowledgement gating, review locking, and restart preserving the message while clearing files. The synthetic test action was blocked by the fixture; no provider call occurred.
- Chrome also verified standalone template editing without a recipient file, save-dialog Escape/focus restoration, completed result detail, and the fixed mobile history link. Captured and inspected 1440x900, 1024x768 and 390x844 layouts, with no document overflow; reduced-motion and viewport overrides were reset. Final healthy-build console checks were empty after correcting an incomplete local results fixture. No Worker/domain/database behavior or live campaign state changed, and no application deployment or real mail test was performed.

### 2026-09-06 - Remaining cleanup integrated onto current main

- Reconciled the earlier cleanup from the `1122ec6` checkout with current main at `328e784`. Main already contains explicit template publication and immutable campaign preparation, identical request retries, shared validation primitives and attachment policy, deleted unused browser rendering/export paths, consolidated CSS, and deferred product/ExcelJS loading. Preserved those newer changes and the FIFO/cancellation, input-hardening, and delivery-evidence safeguards. The original working directory and its design/progress changes remain intact.
- Removed eight remaining unimported re-export entrypoints and unused auth/Graph/attachment aliases. Deleted the unused in-memory campaign/recipient lifecycle; retained `makeSendKey` unchanged and replaced its three unused-model tests with two live send-key tests. Actual lifecycle transitions remain enforced by D1 and the Queue/mailbox code.
- Shared the current SQLite-to-D1 adapter between campaign-create and mailbox-delivery tests in `tests/helpers/sqlite-d1.ts`. Preserved `total_changes()` accounting for trigger writes, real transaction rollback, batch-size assertions, and bounded migration selection for legacy FIFO backfill tests. All current suite assertions remain intact.
- SMTP now imports the existing domain attachment limits instead of defining another five-file/20-MiB pair. Added missing-field reporting to the existing live-renderer assertion and corrected the attachment test's outdated size description. The incremental source/test change is 469 net lines removed across 21 files; overlapping cleanup already present on main was not repeated.
- A clean `npm ci` and `npm run check:staging` passed TypeScript, production/staging client and Worker builds, the initial client bundle budget, all 316 tests across 39 files, both Wrangler packaging dry runs, and isolated staging configuration validation. The one-test net reduction from main's 317 is the unused-model replacement described above. Initial JavaScript remains 83.88 kB gzip; existing dependency annotation and optional ExcelJS chunk warnings remain. `git -c core.safecrlf=false diff --check` passed.
- Started the integrated full-stack preview at `http://127.0.0.1:5176/` and inspected its signed-out landing in the in-app browser. Local HTTP checks passed: landing 200 HTML, unauthenticated `/api/me` 401 JSON, and unknown API GET/POST 404 JSON. No deployment, remote migration, Microsoft authorization, live campaign mutation, attachment operation, or mail submission ran.

## 2026-09-07: Architecture follow-up and recovery foundation

- Reconciled the older architecture assessment with remote main `1b64123` in an isolated worktree, preserving the original checkout and other workstreams. Retained already-shipped draft locking/exact retry, unpublished campaign snapshots, recipients-preserving template reuse, separate failed/unknown outcomes, diagnostics, and deferred route/ExcelJS loading.
- Added mail credential preparation before claims/leases, explicit SMTP/Graph reconnect disposition, atomic proven-rejection settlement and campaign pause, same-owner resume preflight, and a responsive Microsoft reconnect notice. Unknown/accepted evidence remains terminal; cancellation and FIFO handoff remain intact. Reservations use the clock after slow preparation finishes.
- Made template publication a D1 batch with database version allocation, conditional current-pointer/name update and stale-edit conflicts. Updated browser save metadata and removed the separate rename request. Added actual local D1 competing-publication coverage.
- Added deterministic owner-scoped history cursors, recoverable older-page loading, and explicit public campaign field projection. Background service construction now accepts bindings/origin directly. Added read-only aggregate health SQL and capacity/retention/fail-closed restore guidance.
- Validation: `npm run check:staging` passed on the final application code: TypeScript, Worker/client production build, 110 kB initial bundle guard (83.92 kB gzip measured), 346 tests / 43 files, production deployment dry run, isolated staging build/configuration, and staging deployment dry run. Local D1 integration applied every migration without env-file or remote bindings. `git -c core.safecrlf=false diff --check` passed. Vite's deferred ExcelJS size advisory remains.
- Browser: local synthetic fixture checks passed for history failure/retry, distinct reconnect outcomes, desktop/tablet/phone presentation, phone keyboard-accessible history, template conflict, and save-as-new recovery. Fixed the reconnect button wrapping at narrow widths. Final cold startup had no browser warnings/errors; transient context errors during bulk development hot reload cleared on a fresh load. No real authentication, mail, or OneDrive operations ran.
- Cleanup: deleted exactly five stale recipient-job PNG captures and two browser snapshot/log files (890,686 bytes) from ignored local QA folders in the original checkout. The automatic review rejected a combined recursive cleanup command; the narrower seven-file deletion succeeded. Preserved approved mocks, reusable fixtures, secrets, D1 state, and other worktrees. Removed this workstream's temporary preview files/logs after verification.
- See `ARCHITECTURE_FOLLOW_UP_2026-09-07.md` for the assessment resolution matrix. Forward migration 0012 is required before deployment. Remote deployment, multi-mailbox/OneDrive capacity measurements, and a full hosted restore drill are not claimed by this checkpoint.

### 2026-09-11 - Local open-source license setup

- Added the standard root MIT `LICENSE` for Mail Flow's original code and documentation, using Copyright (c) 2026 Kyzer Phneh. Attribution comes from the initial commit `8150b9f`, the current branch's author history, and the shared `kyzer1023` GitHub noreply identity; the repository remote is `kyzer1023/MailFlow`. All recorded commit years are 2026. No legal name or third-party ownership was inferred beyond that repository evidence.
- Checked the license wording against the [Open Source Initiative MIT text](https://opensource.org/license/mit). Added `license: MIT` to `package.json` and the root package entry in `package-lock.json`, retaining `private: true`, every dependency resolution, and all scripts. Added a concise README license section and `THIRD_PARTY_NOTICES.md` with the direct runtime dependency licenses, upstream-notice preservation guidance, and system-font scope. No pre-existing project license or tracked third-party notice was found in the current tree or available Git history.
- Excluded all PNGs in `mock-images/` (including refinement references) and the two PNGs in `public/assets/` from the MIT grant pending provenance and reuse-rights confirmation. Historical records identify the public assets and refinement images as generated from approved references but do not establish the original seven references' creator or reuse terms. No image was changed or claimed as third-party-owned.
- Recorded the existing transitive `buffers@0.1.1` notice gap: `npm explain buffers` confirms `exceljs -> unzipper -> binary -> buffers`; its installed package and lockfile have no license declaration, its files contain no license notice, and its recorded upstream repository returned 404. The project MIT license does not assign terms to that dependency. Resolving its applicable upstream terms and the PNG provenance remains open; dependencies were not replaced in this licensing change.
- Verification: `npm test` passed TypeScript, both production builds, and all 193 tests in 23 files. Existing Zod annotation and large-client-chunk warnings remain. One-off `node` assertions passed canonical MIT text comparison, matching MIT metadata, retention of `private: true`, exact package/lockfile equivalence to HEAD after removing only the new license fields, and local licensing-link resolution. `git -c core.safecrlf=false diff --check` and whitespace checks on the two new notice files passed.
- The checkout was clean at the start. Changes are limited to `LICENSE`, `THIRD_PARTY_NOTICES.md`, `README.md`, `package.json`, `package-lock.json`, and this log. No application behavior or visual layout changed, so no browser or provider check was needed. Changes remain local and uncommitted; no push, deployment, release, or Codex for Open Source application submission ran.

### 2026-09-11 - Licensing integration for origin publication

- Under the user's explicit request to push to origin, applied only licensing commit `dd2d125` on remote main `d4e2f18` in an isolated checkout. The original local main had diverged by 3 local and 35 remote commits. Preserved the original checkout and every newer remote implementation change; resolved the append-only progress conflict by retaining the complete remote log and appending the licensing entry.
- A clean `npm ci --no-audit --no-fund` and `npm test` passed TypeScript, both production builds, the initial client bundle guard (83.92 kB gzip), and all 346 tests in 43 files. Existing dependency deprecation/annotation and deferred ExcelJS chunk warnings remain. Licensing assertions confirmed the six-file scope, unchanged package metadata apart from the MIT field, an unchanged dependency graph, working documentation links, and preservation of the complete remote progress history. `git -c core.safecrlf=false diff --cached --check` passed.
- Prepared a normal fast-forward push to `origin/main`. No application deployment command, migration, provider operation, or Codex for Open Source application submission ran.
