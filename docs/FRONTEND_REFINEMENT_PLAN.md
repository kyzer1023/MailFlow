...
Date: 2026-09-05.
Status: planning brief and engineering recommendation, not an approved API freeze or completed visual audit.
...
## Proposed template saving, selection, and missing-field behavior

The user asked how these interactions would work after accepting the Message screen. The following details are proposals for that continuation, not descriptions of shipped behavior.
...
Existing mapping and validation primitives already detect unavailable columns. The current `hydrateSavedFlow` resets workbook and attachment state, so it cannot be used unchanged by the new post-import chooser. The existing API `createTemplateVersion` helper also advances the flow's current-template pointer whenever a version is created, including preparation for a campaign. Separating explicit template publication from campaign snapshot preparation is therefore a concrete lifecycle contract change needed before promising that one-off edits leave a saved template unchanged. Record the accepted implementation contract in ARCHITECTURE.md or DECISIONS.md before changing it.
