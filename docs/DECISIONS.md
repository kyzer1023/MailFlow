...
Attachment loading is bounded by the reviewed per-file sizes and the 20 MiB set total before bytes are buffered. Deleted or modified OneDrive objects are terminal integrity failures for the immutable set and stop before the next recipient claim. Throttles, service outages, network failures, and interrupted downloads are proven pre-submission failures; running campaigns keep their recipient pending and reserve a delayed attachment-check wake. Terminal and retention cleanup are idempotent, process no more than five objects per set and two sets per scheduled invocation, and never mark a truncated namespace sweep complete. SMTP MIME uses the same five-file and 20 MiB limits with at most 80 KiB output chunks and stable hashed MIME identity for safe pre-submission retries only.
...
## ADR-013 - Coordinate delivery and recovery per authenticated mailbox in D1

Status: accepted for implementation.

D1 is authoritative for every MailFlow provider submission made by one authenticated mailbox, across concurrent campaigns, duplicate Queue deliveries, and self-only test sends. A mailbox coordination row holds one expiring lease, a provider backoff time, and the next mailbox send time. A delivery-attempt ledger records the owning mailbox, campaign job or test-send reference, an unguessable attempt token, the reserved envelope-recipient count, the provider-boundary state, and the terminal accounting result. Lease acquisition and the attempt reservation are one conditional D1 batch transaction, relying on Cloudflare's documented [`D1Database::batch()` transaction and rollback semantics](https://developers.cloudflare.com/d1/worker-api/d1-database/#batch). In-memory locks are never part of the correctness model.

MailFlow enforces an application budget of 8,000 envelope-recipient entries per authenticated mailbox in a rolling 24-hour window. The count includes the primary To recipient, every CC entry, every BCC entry, and the single self recipient used by a test send. Repeated addresses in different or repeated envelope fields are counted once per occurrence rather than deduplicated. This conservative deterministic rule never undercounts what MailFlow asks the provider to process. Accepted and ambiguous or unknown attempts retain their reservation until its 24-hour expiry. An attempt is released only after a result or recovery path proves that no provider submission occurred. The application budget intentionally reserves 2,000 recipients below Microsoft's documented [10,000-recipient rolling mailbox limit](https://learn.microsoft.com/en-us/office365/servicedescriptions/exchange-online-service-description/exchange-online-limits#sending-limits) for ordinary Outlook activity that MailFlow cannot observe.
