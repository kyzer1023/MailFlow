...
- Subject/body dynamic values, escaped spreadsheet values, sanitized HTML, and an isolated preview iframe. Pasted HTML source is treated as markup. The visual editor and Review preview keep email table, image, and link layout instead of applying Mail Flow styles inside the message.
