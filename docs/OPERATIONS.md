...
Promote a reviewed candidate through the production process. Rebuild the production artifact after staging builds and inspect the exact target. Apply only pending production migrations with `npm run db:migrate:remote`; deploy with `npm run deploy` after the gate. The [manual production workflow](../.github/workflows/deploy-production.yml) checks out the supplied commit, applies only production migrations, and deploys that candidate. It uses the existing Cloudflare account token from the GitHub staging environment. Never promote by binding production to staging data or Queues.
...
Verified 2026-09-15 against application commit `e794337` (merged PR 11: HTML editor preview, authored source, and template action pinning). GitHub main also contains the follow-up production workflow commit `d0372f7`, which is not in the Worker bundle.

A local `npm ci` and `npm run check:staging` passed: TypeScript, production and staging builds, 349 tests across 44 files, production dry run, staging configuration validation, and staging dry run. Initial JavaScript is 83.96 kB gzip, below the 110 kB guard. [GitHub verification](https://github.com/kyzer1023/MailFlow/actions/runs/34883581768) passed for `e794337`. Staging was deployed from that same commit. Production migrations reported none pending.

Production runs application commit `e794337` as Worker version `cc4e21ae-514e-4d4b-93aa-7c3c19315c50`, deployed 2026-09-15 (Asia/Kuala_Lumpur). [GitHub production deploy](https://github.com/kyzer1023/MailFlow/actions/runs/34885374592). The production D1, Queue producer/consumer, SMTP configuration and hourly schedule were preserved.

Hosted checks confirmed the landing HTML, JS and CSS hashes match that candidate (`index-CT60okMb.js`, `index-CtXo0DeZ.css`, `TemplatePage-D42JkhAM.js`), `/api/me` 401 JSON, unknown API GET/POST 404 JSON, and the expected Microsoft SMTP OAuth redirect/callback without completing consent. The [public presentation](https://mailflow.kyzer-hono-test.workers.dev/presentation/) returns 200 and requires no sign-in. No live mail was sent by this verification; it does not establish inbox receipt, measured pilot capacity, or a completed hosted restore drill.
