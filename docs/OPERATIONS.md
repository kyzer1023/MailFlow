...
Production runs application commit `0ef2c12` as Worker version `c48b7b3a-4d9f-4bc8-bf09-20fe00164d67`, deployed 2026-09-13 (Asia/Kuala_Lumpur). [GitHub verification](https://github.com/kyzer1023/MailFlow/actions/runs/34710201627) passed for that commit. Forward migrations `0010` through `0012` applied successfully; no migrations remain pending. There were no queued/running campaigns or reserved/provider-bound attempts during promotion. The production D1, Queue producer/consumer, SMTP configuration and hourly schedule were preserved.

Hosted checks confirmed matching hashes for the landing HTML, current JS/CSS and all 11 presentation files, working desktop/mobile navigation from the landing link, `/api/me` 401 JSON, unknown API GET/POST 404 JSON, and the expected Microsoft SMTP OAuth redirect/callback without completing consent. The [public presentation](https://mailflow.kyzer-hono-test.workers.dev/presentation/) requires no sign-in. No live mail was sent by this verification; it does not establish inbox receipt, measured pilot capacity, or a completed hosted restore drill.
