...
export function WizardStepper({ current }: WizardStepperProps) {
  const { validation, skipInvalidRows } = useDraft();
        ...
        {steps.map(([label, to], index) => {
          ...
          const content = (
            <>
              <span className="stepper-node" aria-hidden="true">
                {state === "complete" &&
                !(index === 0 && attention && !skipInvalidRows) ? (
                  <Check weight="bold" />
                ) : (
                  index + 1
                )}
              ...
              <span className="stepper-label">
                {label}
                {index === 0 && current > 0 && attention > 0 && (
                  <small
                    className={skipInvalidRows ? "stepper-skipped" : undefined}
                  >
                    {attention} {skipInvalidRows ? "skipped" : "to review"}
                  </small>
                )}
              </span>
            </>
          ...
          return (
            <li className={state} key={label}>
              {state === "future" ? (
                <span className="stepper-future">{content}</span>
              ) : (
                <Link
                  to={to}
