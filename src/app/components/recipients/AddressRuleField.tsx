...
export function AddressRuleField({ fieldKey, label, value, mode, column, options, onValue, onMode, onColumn, hint }: AddressRuleFieldProps) {
  ...
  useEffect(() => {
    ...
    const close = (event: Event) => { if (!fieldRef.current?.contains(event.target as Node)) setMenuOpen(false); };
  ...
  const removeAddress = (index: number) => onValue(addresses.filter((_, itemIndex) => itemIndex !== index).join("; "));
