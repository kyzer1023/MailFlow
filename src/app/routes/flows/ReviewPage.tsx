...
type TestState = "idle" | "sending" | "accepted" | "replayed" | "error";
...
export function ReviewPage() {
  ...
  const fallbackTestRequest = useRef<Parameters<typeof sendCampaignTest>[1] | null>(null);
  ...
  const previews = useMemo(() => buildMessagePreviews({ senderAddress: sender, subjectTemplate: state.draft.subject, bodyHtml: state.bodyHtml, rows, fieldMappings: state.mapping.placeholders }), [sender, state.draft.subject, state.mapping.placeholders, state.bodyHtml, rows]);
  ...
  const testActionBlocker = blockingIssues.length > 0
    ? `Resolve ${blockingIssues.length} validation ${blockingIssues.length === 1 ? "issue" : "issues"} first.`
    : attachmentBlocker;
  const startActionBlocker = testActionBlocker || (!ack ? "Check the final acknowledgement first." : "");
  const blockerDescriptionId = testActionBlocker ? "review-blockers" : undefined;
  const actionPending = testState === "sending" || startState === "starting";
  const testComplete = testState === "accepted" || testState === "replayed";

  const sendTest = async () => {
    if (actionInFlightRef.current || testComplete || !message || !state.campaignValidation?.ok || !state.attachmentsReady) return;
    actionInFlightRef.current = "test";
    ...
    try {
      ...
      const testResponse = await sendCampaignTest(response!.campaign.id, testRequest.current, csrfToken);
      setTestState(testResponse.replayed ? "replayed" : "accepted");
    } catch (error) {
      setTestState("error");
  ...
  const start = async () => {
    ...
    actionInFlightRef.current = "start";
  ...
  if (!state.table) {
    return <AppShell><div className="route-gate" role="status"><WarningCircle weight="fill" /><h1>Import a recipient file first.</h1><p>Review only becomes available after Recipients and Message are complete.</p><Link className="button button--coral" to="/flows/new/data">Start with data</Link></div></AppShell>;
  }

  const previewDocument = message ? buildPreviewSrcDoc(message.bodyHtml) : "";
  const summaryFacts: readonly (readonly [Icon, string, string])[] = [[Envelope, "Sender", sender], [Users, "Recipients", `${state.validation?.validRecipientCount ?? 0} ready, ${state.validation?.skippedRecipientCount ?? 0} ${state.skipInvalidRows ? "skipped" : "need attention"}`], [Envelope, "CC", state.draft.ccMode === "column" ? `From ${state.draft.ccColumn}` : state.draft.cc || "None"], [Gauge, "Importance", state.draft.importance === "high" ? "High" : state.draft.importance === "low" ? "Low" : "Normal"], [Paperclip, "Attachments", attachmentSummaryText(attachments)], [Gauge, "Pacing", `${state.draft.pace} messages per minute`], [Clock, "Estimated duration", `About ${Math.ceil((state.validation?.validRecipientCount ?? 0) / state.draft.pace)} minutes`], [CheckCircle, "Validation", state.campaignValidation?.ok ? "Ready to queue" : `${blockingIssues.length} issues to review`]];
  const testActionLabel = testState === "sending"
    ? "Sending test..."
    : testState === "accepted"
      ? "Test accepted"
      : testState === "replayed"
